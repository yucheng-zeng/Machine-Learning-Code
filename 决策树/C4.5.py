...
# 用信息增益比作为选择特征的条件
# 选择最好的划分方式(选取每个特征划分数据集,从中选取信息增益比最大的作为最优划分,即选取信息增益比的最大的特征）
def chooseBest(dataSet, labels):
    featNum = len(labels) - 1  # 最后一列是分类, 不能作为特征
    baseEntropy = calEmpiricalEntropy(dataSet)  # 经验熵
    bestInforGainRating = 0.0  # 最好的信息增益的数值
    bestFeature = -1  # 表示最好划分特征的下标

    for i in range(featNum):  # 遍历所有特征, 计算每一个特征的信息增益
        inforGainRating = calInformationGainRate(dataSet,baseEntropy,i)
        if (inforGainRating > bestInforGainRating):  # 判断, 择优
            bestInforGainRating = inforGainRating
...
def classify(inputTree,featLabels,testVec):  # 传入的数据为dict类型
    firstSides = list(inputTree.keys())  # 第一个结点的特征
    #print(firstSides)
...
dataSet, labels = readDataFromFile('lenses1.txt')
labelsCopy = labels[:]
mytree = createTree(dataSet, labels)
labels = labels[:]
createPlot(mytree)